...
public class AirportManager {
    ...
    // Получение аппарата по индексу (1-based)
    ...
    // Аренда выбранного аппарата с проверкой ТО
    public void rentAircraft(int index, int days) {
        Aircraft a = getAircraft(index);
        if (a == null) {
            System.out.println("Аппарата с таким индексом не существует.");
            return;
        }
        if (a instanceof Maintainable && a.needsMaintenance()) {
            System.out.println("Аппарат требует технического обслуживания.");
            log.warn("Попытка аренды без ТО: " + a.getModel());
            return;
        }

        if (a instanceof Rentable) {
            ((Rentable) a).rent(days);
            log.info("Аппарат '" + a.getModel() + "' успешно арендован на " + days + " дней");
            System.out.println("[OK] Аренда оформлена!");
        } else {
            System.out.println("(!) Этот аппарат не поддерживает аренду.");
        }
    }
    // ВЫВОД СПИСКА С ИНДЕКСАМИ (для пункта 19)
    public void showAllWithIndexes() {
        if (fleet.isEmpty()) {
            System.out.println("Аэропорт пуст.");
            return;
        }
        System.out.println("\n=== СПИСОК АППАРАТОВ С ИНДЕКСАМИ ===");
        for (int i = 0; i < fleet.size(); i++) {
            System.out.print((i + 1) + ". ");
            fleet.get(i).introduce();
        }
    }

    // ТО ДЛЯ ВЫБРАННОГО АППАРАТА (новый пункт 20)
    public void performMaintenanceOn(int index) {
        Aircraft a = getAircraft(index);
        if (a == null) {
        ...
        if (a instanceof Maintainable) {
            ((Maintainable) a).performMaintenance();
            a.markAsMaintained();
            log.info("Выполнено ТО для выбранного аппарата: " + a.getModel());
            System.out.println("Техническое обслуживание выполнено!");
        } else {
            System.out.println("Этот аппарат не требует технического обслуживания (дрон).");
        }
    }
}